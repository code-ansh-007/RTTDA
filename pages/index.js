...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="flex flex-col items-center justify-center mt-5 px-3">
        ...
        >
          ...
          {todoLoading ? (
            <div>
              <i className="fa-solid text-7xl animate-spin text-blue-500 fa-spinner"></i>
            </div>
          ) : todos.length > 0 ? (
            todos.map((item) => {
              return (
                <Todo key={item.id} todo={item.data().task} id={item.id} />
              );
            })
          ) : (
            <div className="flex flex-col items-center mt-[20vh] sm:mt-[14vh]">
              <Image src={thinkDog} width={200} alt="thinking dog pic" />
